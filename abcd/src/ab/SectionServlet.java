...
public class SectionServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("sectionservlet Called");
		 ...
		 try
		 {
			 ...
			 Cnnt con= new Cnnt();
			PreparedStatement st= con.getDbConnection().prepareStatement("select * from section where subdivision_id=?");
			st.setInt(1, x);
			ResultSet rs= st.executeQuery();
			
			while(rs.next())
			{
					arr.add(rs.getString("section_name" ));
					arr1.add(rs.getString("section_id" ));
			}
