...
public class CircleServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 System.out.println("cirservlet Called");
		 ...
		 try
		 {
			 ...
			 Cnnt con= new Cnnt();
			PreparedStatement st= con.getDbConnection().prepareStatement("select * from circle where company_id=?");
			st.setInt(1, x);
			ResultSet rs= st.executeQuery();
			
			while(rs.next())
			{
					arr.add(rs.getString("circle_name" ));
					arr1.add(rs.getString("circle_id" ));
