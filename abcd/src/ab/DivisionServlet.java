...
public class DivisionServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("divisionservlet Called");
		 ...
		 try
		 {
			 ...
			 Cnnt con= new Cnnt();
			PreparedStatement st= con.getDbConnection().prepareStatement("select * from division where circle_id=?");
			st.setInt(1, x);
			ResultSet rs= st.executeQuery();
			
			while(rs.next())
			{
					arr.add(rs.getString("division_name" ));
					arr1.add(rs.getString("division_id" ));
			}
